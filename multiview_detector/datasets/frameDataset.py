...
class frameDataset(VisionDataset):
    def __init__(
        ...
        super().__init__(
            base.root, transform=transform, target_transform=target_transform
        )
        ...
        self.reducedgrid_shape = list(
            map(lambda x: int(x / self.grid_reduce), self.worldgrid_shape)
        )
        ...
        self.map_kernel = torch.zeros(
            [1, 1, kernel_size, kernel_size], requires_grad=False
        )
        ...
        self.img_kernel = torch.zeros(
            [2, 2, kernel_size, kernel_size], requires_grad=False
        )
    ...
    def prepare_gt(self):
        og_gt = []
        for fname in sorted(
            os.listdir(os.path.join(self.root, "annotations_positions_old"))
        ):
            frame = int(fname.split(".")[0])
            with open(
                os.path.join(self.root, "annotations_positions_old", fname)
            ) as json_file:
    ...
    def download(self, frame_range):
        for fname in sorted(
            ...
            if frame in frame_range:
                ...
                foot_row_cam_s, foot_col_cam_s, v_cam_s = (
                    [[] for _ in range(self.num_cam)],
                    [[] for _ in range(self.num_cam)],
                    [[] for _ in range(self.num_cam)],
                )
                for single_pedestrian in all_pedestrians:
                    x, y = self.base.get_worldgrid_from_pos(
                        single_pedestrian["positionID"]
                    )
                    ...
                    for cam in range(self.num_cam):
                        x = max(
                            min(
                                int(
                                    (
                                        single_pedestrian["views"][cam]["xmin"]
                                        + single_pedestrian["views"][cam]["xmax"]
                                    )
                                    / 2
                                ),
                                self.img_shape[1] - 1,
                        ...
                        y_foot = min(
                            single_pedestrian["views"][cam]["ymax"],
                            self.img_shape[0] - 1,
                        )
                        if x > 0 and y > 0:
                            ...
                            v_cam_s[cam].append(
                                single_pedestrian["personID"] + 1 if self.reID else 1
                            )
                occupancy_map = coo_matrix(
                    (v_s, (i_s, j_s)), shape=self.reducedgrid_shape
                )
                self.map_gt[frame] = occupancy_map
                self.imgs_head_foot_gt[frame] = {}
                for cam in range(self.num_cam):
                    img_gt_head = coo_matrix(
                        (v_cam_s[cam], (head_row_cam_s[cam], head_col_cam_s[cam])),
                        shape=self.img_shape,
                    )
                    img_gt_foot = coo_matrix(
                        (v_cam_s[cam], (foot_row_cam_s[cam], foot_col_cam_s[cam])),
                        shape=self.img_shape,
                    )
